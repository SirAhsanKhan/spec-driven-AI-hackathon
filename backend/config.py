...
# Use PostgreSQL (Neon) in production (Vercel), SQLite for local development
VERCEL_ENV = os.getenv("VERCEL_ENV")
if VERCEL_ENV:
    # In Vercel, require a proper database URL (PostgreSQL, such as Neon)
    DATABASE_URL = os.getenv("DATABASE_URL")
    # Ensure we're using PostgreSQL, not SQLite
    if not DATABASE_URL or "sqlite" in DATABASE_URL.lower():
        raise ValueError(
            "SQLite is not supported in Vercel. Please configure a PostgreSQL database "
            "(like Neon) and set the DATABASE_URL environment variable."
        )
    # Add connection parameters for Neon/PostgreSQL in serverless environments
    if "postgresql" in DATABASE_URL.lower() or "postgres" in DATABASE_URL.lower():
        # For Neon or other PostgreSQL databases in serverless environments
